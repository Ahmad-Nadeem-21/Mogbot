...
- [x] Host Flask with `ANTHROPIC_API_KEY` as an environment variable, never in code. **Live on Render** at `https://mogbot-i1ts.onrender.com`, deployed by hand through the dashboard. `render.yaml` now makes that reproducible via "New +" -> "Blueprint" instead of manual clicks (not yet applied through the Blueprint flow itself - the live instance predates it). The **Terraform stack in `terraform/`** (AWS: Lightsail + CloudFront) remains a ready, unapplied alternative if you ever want off Render.
...
- [ ] Push `web/` to a static host. Three ready options: Netlify/Vercel/GitHub Pages (manual, plain files); `render.yaml`'s `mogbot-frontend` static-site service (same Blueprint as the backend, one deploy for both); or the **Terraform stack in `terraform/`** (AWS: private S3 + CloudFront via OAC) - `terraform/scripts/deploy_frontend.sh` syncs `web/` and invalidates the cache automatically. None applied yet.
